...
fn test_data_dir_path() {
    ...
    cfg_if::cfg_if! {
        if #[cfg(target_os = "macos")] {
            assert_eq!(data_dir(), home_dir.join(".war"));
        } else if #[cfg(any(target_os = "linux", target_os = "freebsd"))] {
            assert_eq!(data_dir(), home_dir.join(".local/share/war"));
        } else if #[cfg(windows)] {
            assert_eq!(data_dir(), home_dir.join("AppData\\Roaming\\oaix\\War\\data"));
        } else {
...
fn test_config_local_dir_path() {
    ...
    cfg_if::cfg_if! {
        if #[cfg(target_os = "macos")] {
            assert_eq!(config_local_dir(), home_dir.join(".war"));
        } else if #[cfg(any(target_os = "linux", target_os = "freebsd"))] {
            assert_eq!(config_local_dir(), home_dir.join(".config/war"));
        } else if #[cfg(windows)] {
            assert_eq!(config_local_dir(), home_dir.join("AppData\\Local\\oaix\\War\\config"));
        } else {
            unimplemented!("Need to update tests for current platform!");
        }
    }
...
fn test_macos_config_dir_name_scopes_to_data_profile() {
    ...
    assert_eq!(macos_config_dir_name_for(Channel::Oss, None), ".war");
    assert_eq!(
...
fn test_gui_app_id_maps_oss_tui_to_oss_gui() {
    ...
    assert_eq!(gui_app_id.to_string(), "tech.oaix.War");
}
...
fn test_gui_config_and_mcp_paths_resolve_explicit_sources() {
    ...
    cfg_if::cfg_if! {
        if #[cfg(target_os = "macos")] {
            assert_eq!(gui_config_dir, home_dir.join(".war"));
        } else if #[cfg(any(target_os = "linux", target_os = "freebsd"))] {
            assert_eq!(gui_config_dir, home_dir.join(".config/war"));
        } else if #[cfg(windows)] {
            assert_eq!(
                gui_config_dir,
                home_dir.join("AppData\\Local\\oaix\\War\\config")
            );
        } else {
            unimplemented!("Need to update tests for current platform!");
        }
...
fn test_warp_home_config_dir_path() {
    ...
    let expected_dir_name = match ChannelState::data_profile() {
        Some(data_profile) => format!(".war-{data_profile}"),
        None => ".war".to_string(),
    };
...
fn test_cache_dir_path() {
    ...
    cfg_if::cfg_if! {
        if #[cfg(target_os = "macos")] {
            assert_eq!(cache_dir(), home_dir.join("Library/Application Support/tech.oaix.War"));
        } else if #[cfg(any(target_os = "linux", target_os = "freebsd"))] {
            assert_eq!(cache_dir(), home_dir.join(".cache/war"));
        } else if #[cfg(windows)] {
            assert_eq!(cache_dir(), home_dir.join("AppData\\Local\\oaix\\War\\cache"));
        } else {
            unimplemented!("Need to update tests for current platform!");
        }
    }
...
fn test_state_dir_path() {
    ...
    cfg_if::cfg_if! {
        // ChannelState, by default, is configured for Channel::Oss.
        if #[cfg(target_os = "macos")] {
            assert_eq!(state_dir(), home_dir.join("Library/Application Support/tech.oaix.War"));
        } else if #[cfg(any(target_os = "linux", target_os = "freebsd"))] {
            assert_eq!(state_dir(), home_dir.join(".local/state/war"));
        } else if #[cfg(windows)] {
            assert_eq!(state_dir(), home_dir.join("AppData\\Local\\oaix\\War\\data"));
        } else {
            unimplemented!("Need to update tests for current platform!");
        }
    }
...
fn test_project_path_for_oss_app_id() {
    let project_dirs = project_dirs_for_app_id(AppId::new("tech", "oaix", "War"), None)
        .expect("should be able to compute project dirs");
    cfg_if::cfg_if! {
        if #[cfg(target_os = "macos")] {
            assert_eq!(project_dirs.project_path(), "tech.oaix.War");
        } else if #[cfg(any(target_os = "linux", target_os = "freebsd"))] {
            assert_eq!(project_dirs.project_path(), "war");
        } else if #[cfg(windows)] {
            assert_eq!(project_dirs.project_path(), "oaix\\War");
        } else {
            unimplemented!("Need to update tests for current platform!");
        }
    }
...
#[test]
fn test_project_local_config_convention_remains_warp() {
    assert_eq!(WARP_CONFIG_DIR, ".warp");
}

#[cfg(target_os = "macos")]
#[test]
fn test_oss_does_not_use_warp_app_group() {
    assert!(app_group_container_path().is_none());
    assert!(secure_state_dir().is_none());
}
